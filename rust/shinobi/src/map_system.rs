...
use sekai::{EntityLookupSystem, EntityUid, GameStateMapData, GridId, MapId, PhysicsQueryHit, TileRef};
...
impl MapSystem {
    ...
    pub fn grid_entity(&self, entities: &ClientEntityManager, grid_id: GridId) -> Option<EntityUid> {
        entities
            .inner
            .map_grid_components
            .iter()
    ...
    pub fn grid_world_position(&self, entities: &ClientEntityManager, grid_id: GridId) -> Option<Vector2> {
        let uid = self.grid_entity(entities, grid_id)?;
        entities.inner.map_grids.get(&uid).map(|grid| grid.world_position)
    }

    pub fn grid_world_bounds(&self, entities: &ClientEntityManager, grid_id: GridId) -> Option<Box2> {
        let uid = self.grid_entity(entities, grid_id)?;
        entities.inner.map_grids.get(&uid).map(|grid| grid.world_bounds())
    }
    ...
    pub fn tile_ref(
        ...
        let uid = self.grid_entity(entities, grid_id)?;
        entities.inner.map_grids.get(&uid).map(|grid| grid.get_tile_ref(tile_indices))
    }
...
mod tests {
    ...
    use sekai::{BodyType, ChunkDatum, EntityUid, GameStateMapData, GridDatum, GridId, MapCoordinates, MapId, Tile, TileRenderFlag};
    ...
    fn map_system_applies_grid_state() {
        ...
        assert_eq!(system.grid_world_position(&entities, GridId::new(4)), Some(Vector2::ZERO));
        assert_eq!(
    ...
    fn map_system_can_query_tile_refs_bounds_and_entities() {
        ...
        assert_eq!(system.tile_ref(&entities, GridId::new(4), Vector2i::new(0, 0)).unwrap().tile.type_id, 2);
        assert!(system
            .tile_world_bounds(&entities, GridId::new(4), Vector2i::new(0, 0))
            .unwrap()
            .contains(Vector2::new(0.5, 0.5), true));
        assert_eq!(system.entities_at_tile(&entities, GridId::new(4), Vector2i::new(0, 0)), vec![uid]);
        assert_eq!(
            system.entities_at_tiles(&entities, GridId::new(4), [Vector2i::new(0, 0), Vector2i::new(0, 0)]),
            vec![uid]
        );
        assert_eq!(
            system.entities_in_grid_aabb(&entities, GridId::new(4), Box2::new(-1.0, -1.0, 1.0, 1.0), false),
            vec![uid]
        );
        assert!(system.entity_world_aabb(&entities, uid).unwrap().contains(Vector2::new(0.5, 0.5), true));
        assert_eq!(
            system.entities_in_map_aabb(&entities, &physics, MapId::new(1), Box2::new(-1.0, -1.0, 1.0, 1.0)),
            vec![uid]
        );
        assert_eq!(
            system
    ...
    fn map_system_can_query_anchored_entities_after_runtime_rebuild() {
        ...
        assert_eq!(system.entities_at_tile(&entities, GridId::new(4), Vector2i::new(0, 0)), vec![uid]);
        assert_eq!(
            system.entities_in_grid_aabb(&entities, GridId::new(4), Box2::new(-1.0, -1.0, 1.0, 1.0), true),
            vec![uid]
        );
    }
