...
mod tests {
    ...
    fn server_game_state_manager_builds_updates_for_in_game_sessions() {
        ...
        let updates = states.send_game_state_update(&mut entities, &mut maps, &mut players, GameTick::new(10));
        assert_eq!(updates.len(), 1);
        assert_eq!(updates[0].1.to_sequence, GameTick::new(10));
        assert!(updates[0].1.entity_states.iter().any(|state| state.uid == uid));
        assert!(updates[0].1.map_data.is_some());

        states.ack("u1", GameTick::new(10));
        let updates = states.send_game_state_update(&mut entities, &mut maps, &mut players, GameTick::new(11));
        ...
        let updates = states.send_game_state_update(&mut entities, &mut maps, &mut players, GameTick::new(12));
        assert_eq!(updates[0].1.entity_states.len(), 1);

        maps.mark_grid_moved(map_id, grid_id);
        players.set_current_tick(GameTick::new(13));
        let updates = states.send_game_state_update(&mut entities, &mut maps, &mut players, GameTick::new(13));
    ...
    fn server_game_state_manager_sends_deleted_grids_incrementally() {
        ...
        let updates = states.send_game_state_update(&mut entities, &mut maps, &mut players, GameTick::new(1));
        assert!(updates[0].1.map_data.as_ref().unwrap().grid_data.contains_key(&grid_id));
    ...
    fn server_game_state_manager_sends_only_changed_chunks_after_ack() {
        ...
        assert_eq!(initial.grid_data[&grid_id].chunk_data[0].index, keisan::Vector2i::new(0, 0));
        ...
        assert_eq!(changed.grid_data[&grid_id].chunk_data[0].index, keisan::Vector2i::new(1, 0));
    }
    ...
    fn server_game_state_manager_sends_deleted_chunks_incrementally() {
        ...
        assert_eq!(changed.grid_data[&grid_id].chunk_data[0].index, keisan::Vector2i::new(0, 0));
    }
    ...
    fn server_game_state_manager_sends_incremental_player_states_and_disconnects() {
        ...
        assert_eq!(updates[0].1.player_states.len(), 1);
        assert_eq!(updates[0].1.player_states[0].user_id, "u2");
        assert_eq!(updates[0].1.player_states[0].status, sekai::SessionStatus::Disconnected);
    }
