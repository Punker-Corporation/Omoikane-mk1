...
use sekai::{EntityLookupSystem, EntityUid, GridId, MapGrid, MapId, MapManager, PhysicsQueryHit, TileRef};
...
impl MapSystem {
    ...
    pub fn map_entity(&self, maps: &MapManager, map: MapId) -> Option<EntityUid> {
        (map != MapId::NULLSPACE).then(|| maps.get_map_entity_id(map)).filter(|uid| uid.is_valid())
    }
    ...
    pub fn grid(&self, maps: &MapManager, entities: &ServerEntityManager, grid_id: GridId) -> Option<MapGrid> {
    ...
    pub fn tile_world_bounds(
        ...
        self.lookup.get_world_bounds(&entities.inner, tile_ref, None)
    }
    ...
    pub fn entities_in_grid_aabb(
        ...
        self.lookup
            .get_entities_intersecting_world_aabb(&entities.inner, grid_uid, world_aabb, include_anchored)
    }
...
mod tests {
    ...
    fn map_system_can_query_and_move_grids() {
        ...
        assert_eq!(system.map_entity(&maps, map_id), Some(maps.get_map_entity_id(map_id)));
        assert_eq!(system.grid_entity(&maps, grid_id), Some(grid_uid));
        assert_eq!(system.try_find_grid_at(&maps, &entities, map_id, Vector2::ZERO), Some(grid_id));
        assert_eq!(
            system.find_grids_intersecting(&maps, &entities, map_id, Box2::new(-1.0, -1.0, 1.0, 1.0), false),
            vec![grid_id]
        );

        let mut transforms = TransformSystem::new();
        assert!(system.set_grid_position(&maps, &mut entities, &mut transforms, grid_id, Vector2::new(2.0, 0.0)));
        assert!(system.set_grid_rotation(&maps, &mut entities, &mut transforms, grid_id, Angle::from_degrees(30.0)));
    ...
    fn map_system_can_query_tiles_and_entities() {
        ...
        let system = MapSystem::new();
        assert_eq!(system.tile_ref(&maps, &entities, grid_id, Vector2i::new(0, 0)).unwrap().tile.type_id, 1);
        assert_eq!(system.entities_at_tile(&entities, grid_id, Vector2i::new(0, 0)), vec![uid]);
        assert_eq!(
            system.entities_at_tiles(&entities, grid_id, [Vector2i::new(0, 0), Vector2i::new(0, 0)]),
            vec![uid]
        );
        assert_eq!(
            system.entities_in_grid_aabb(&maps, &entities, grid_id, Box2::new(-1.0, -1.0, 1.0, 1.0), false),
            vec![uid]
        );
        assert!(system
        ...
        assert!(system.entity_world_aabb(&entities, uid).unwrap().contains(Vector2::new(0.5, 0.5), true));
        assert_eq!(
            system.entities_in_map_aabb(&maps, &entities, &physics, map_id, Box2::new(-1.0, -1.0, 1.0, 1.0)),
            vec![uid]
        );
        assert_eq!(
            system
    ...
    fn map_system_can_query_anchored_entities_in_tiles_and_grid_aabbs() {
        ...
        assert_eq!(system.entities_at_tile(&entities, grid_id, Vector2i::new(0, 0)), vec![uid]);
        assert_eq!(
            system.entities_in_grid_aabb(&maps, &entities, grid_id, Box2::new(-1.0, -1.0, 1.0, 1.0), true),
            vec![uid]
        );
    }
    ...
    fn map_system_intersect_ray_respects_collision_mask() {
        ...
        let mut first_fixture =
            butsuri::Fixture::new("first", butsuri::PhysShape::Aabb(butsuri::AabbShape::new(Box2::new(-1.0, -1.0, 1.0, 1.0), 0.0)));
        first_fixture.collision_layer = 1 << 0;
        entities.inner.ensure_fixtures(first).insert_fixture(first_fixture);
        ...
        let mut second_fixture =
            butsuri::Fixture::new("second", butsuri::PhysShape::Aabb(butsuri::AabbShape::new(Box2::new(-1.0, -1.0, 1.0, 1.0), 0.0)));
        second_fixture.collision_layer = 1 << 1;
        entities.inner.ensure_fixtures(second).insert_fixture(second_fixture);
