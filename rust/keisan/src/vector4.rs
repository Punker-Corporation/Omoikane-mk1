use crate::{ApproxEq, MathHelper, Vector2, Vector3};
use core::fmt;
use core::ops::{Add, Div, Mul, Neg, Sub};

#[derive(Clone, Copy, Default)]
pub struct Vector4 {
    pub x: f32,
    pub y: f32,
    pub z: f32,
    pub w: f32,
}

impl Vector4 {
    pub const ZERO: Self = Self::new(0.0, 0.0, 0.0, 0.0);
    pub const ONE: Self = Self::new(1.0, 1.0, 1.0, 1.0);
    pub const UNIT_X: Self = Self::new(1.0, 0.0, 0.0, 0.0);
    pub const UNIT_Y: Self = Self::new(0.0, 1.0, 0.0, 0.0);
    pub const UNIT_Z: Self = Self::new(0.0, 0.0, 1.0, 0.0);
    pub const UNIT_W: Self = Self::new(0.0, 0.0, 0.0, 1.0);

    pub const fn new(x: f32, y: f32, z: f32, w: f32) -> Self {
        Self { x, y, z, w }
    }

    pub const fn splat(value: f32) -> Self {
        Self::new(value, value, value, value)
    }

    pub fn xy(self) -> Vector2 {
        Vector2::new(self.x, self.y)
    }

    pub fn xyz(self) -> Vector3 {
        Vector3::new(self.x, self.y, self.z)
    }

    pub fn length(self) -> f32 {
        self.length_squared().sqrt()
    }

    pub fn length_squared(self) -> f32 {
        self.x * self.x + self.y * self.y + self.z * self.z + self.w * self.w
    }

    pub fn normalized(self) -> Self {
        self / self.length()
    }

    pub fn normalize(&mut self) {
        *self = self.normalized();
    }

    pub fn component_min(a: Self, b: Self) -> Self {
        Self::new(a.x.min(b.x), a.y.min(b.y), a.z.min(b.z), a.w.min(b.w))
    }

    pub fn component_max(a: Self, b: Self) -> Self {
        Self::new(a.x.max(b.x), a.y.max(b.y), a.z.max(b.z), a.w.max(b.w))
    }

    pub fn clamp(vector: Self, min: Self, max: Self) -> Self {
        Self::new(
            MathHelper::clamp(vector.x, min.x, max.x),
            MathHelper::clamp(vector.y, min.y, max.y),
            MathHelper::clamp(vector.z, min.z, max.z),
            MathHelper::clamp(vector.w, min.w, max.w),
        )
    }

    pub fn dot(a: Self, b: Self) -> f32 {
        a.x * b.x + a.y * b.y + a.z * b.z + a.w * b.w
    }

    pub fn lerp(a: Self, b: Self, factor: f32) -> Self {
        Self::new(
            MathHelper::lerp(a.x, b.x, factor),
            MathHelper::lerp(a.y, b.y, factor),
            MathHelper::lerp(a.z, b.z, factor),
            MathHelper::lerp(a.w, b.w, factor),
        )
    }

    pub fn barycentric(a: Self, b: Self, c: Self, u: f32, v: f32) -> Self {
        a + (b - a) * u + (c - a) * v
    }

    pub fn interpolate_cubic(pre_a: Self, a: Self, b: Self, post_b: Self, t: f32) -> Self {
        a + (b - pre_a
            + (pre_a * 2.0 - a * 5.0 + b * 4.0 - post_b + ((a - b) * 3.0 + post_b - pre_a) * t) * t)
            * t
            * 0.5
    }
}

impl PartialEq for Vector4 {
    fn eq(&self, other: &Self) -> bool {
        self.x == other.x && self.y == other.y && self.z == other.z && self.w == other.w
    }
}

impl fmt::Debug for Vector4 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "({}, {}, {}, {})", self.x, self.y, self.z, self.w)
    }
}

impl fmt::Display for Vector4 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "({}, {}, {}, {})", self.x, self.y, self.z, self.w)
    }
}

impl Add for Vector4 {
    type Output = Self;

    fn add(self, rhs: Self) -> Self::Output {
        Self::new(
            self.x + rhs.x,
            self.y + rhs.y,
            self.z + rhs.z,
            self.w + rhs.w,
        )
    }
}

impl Add<f32> for Vector4 {
    type Output = Self;

    fn add(self, rhs: f32) -> Self::Output {
        Self::new(self.x + rhs, self.y + rhs, self.z + rhs, self.w + rhs)
    }
}

impl Sub for Vector4 {
    type Output = Self;

    fn sub(self, rhs: Self) -> Self::Output {
        Self::new(
            self.x - rhs.x,
            self.y - rhs.y,
            self.z - rhs.z,
            self.w - rhs.w,
        )
    }
}

impl Sub<f32> for Vector4 {
    type Output = Self;

    fn sub(self, rhs: f32) -> Self::Output {
        Self::new(self.x - rhs, self.y - rhs, self.z - rhs, self.w - rhs)
    }
}

impl Neg for Vector4 {
    type Output = Self;

    fn neg(self) -> Self::Output {
        Self::new(-self.x, -self.y, -self.z, -self.w)
    }
}

impl Mul<f32> for Vector4 {
    type Output = Self;

    fn mul(self, rhs: f32) -> Self::Output {
        Self::new(self.x * rhs, self.y * rhs, self.z * rhs, self.w * rhs)
    }
}

impl Mul for Vector4 {
    type Output = Self;

    fn mul(self, rhs: Self) -> Self::Output {
        Self::new(
            self.x * rhs.x,
            self.y * rhs.y,
            self.z * rhs.z,
            self.w * rhs.w,
        )
    }
}

impl Div<f32> for Vector4 {
    type Output = Self;

    fn div(self, rhs: f32) -> Self::Output {
        Self::new(self.x / rhs, self.y / rhs, self.z / rhs, self.w / rhs)
    }
}

impl Div for Vector4 {
    type Output = Self;

    fn div(self, rhs: Self) -> Self::Output {
        Self::new(
            self.x / rhs.x,
            self.y / rhs.y,
            self.z / rhs.z,
            self.w / rhs.w,
        )
    }
}

impl From<(f32, f32, f32, f32)> for Vector4 {
    fn from(value: (f32, f32, f32, f32)) -> Self {
        Self::new(value.0, value.1, value.2, value.3)
    }
}

impl From<f32> for Vector4 {
    fn from(value: f32) -> Self {
        Self::splat(value)
    }
}

impl From<Vector2> for Vector4 {
    fn from(value: Vector2) -> Self {
        Self::new(value.x, value.y, 0.0, 0.0)
    }
}

impl From<Vector3> for Vector4 {
    fn from(value: Vector3) -> Self {
        Self::new(value.x, value.y, value.z, 0.0)
    }
}

impl From<(Vector3, f32)> for Vector4 {
    fn from(value: (Vector3, f32)) -> Self {
        Self::new(value.0.x, value.0.y, value.0.z, value.1)
    }
}

impl ApproxEq for Vector4 {
    fn approx_eq(&self, other: Self) -> bool {
        MathHelper::close_to(self.x, other.x, 0.000_000_1)
            && MathHelper::close_to(self.y, other.y, 0.000_000_1)
            && MathHelper::close_to(self.z, other.z, 0.000_000_1)
            && MathHelper::close_to(self.w, other.w, 0.000_000_1)
    }

    fn approx_eq_with_tolerance(&self, other: Self, tolerance: f64) -> bool {
        MathHelper::close_to_f64(self.x as f64, other.x as f64, tolerance)
            && MathHelper::close_to_f64(self.y as f64, other.y as f64, tolerance)
            && MathHelper::close_to_f64(self.z as f64, other.z as f64, tolerance)
            && MathHelper::close_to_f64(self.w as f64, other.w as f64, tolerance)
    }
}

#[cfg(test)]
mod tests {
    use super::Vector4;
    use crate::{ApproxEq, MathHelper, Vector2, Vector3};

    #[test]
    fn vector4_arithmetic_and_norms_match_expected_shape() {
        let a = Vector4::new(2.0, 4.0, 4.0, 10.0);
        let b = Vector4::new(1.0, -2.0, 5.0, -6.0);
        assert_eq!(a + b, Vector4::new(3.0, 2.0, 9.0, 4.0));
        assert_eq!(a - b, Vector4::new(1.0, 6.0, -1.0, 16.0));
        assert_eq!(a * 2.0, Vector4::new(4.0, 8.0, 8.0, 20.0));
        assert_eq!(a / 2.0, Vector4::new(1.0, 2.0, 2.0, 5.0));
        assert!(MathHelper::close_to(a.length(), 11.661_903, 0.000_001));
    }

    #[test]
    fn vector4_dot_lerp_and_clamp_match_csharp_semantics() {
        let a = Vector4::UNIT_X;
        let b = Vector4::ONE;
        assert_eq!(Vector4::dot(a, b), 1.0);
        assert_eq!(
            Vector4::lerp(Vector4::ZERO, Vector4::ONE, 0.5),
            Vector4::splat(0.5)
        );
        assert_eq!(
            Vector4::clamp(
                Vector4::new(-1.0, 0.5, 2.0, 8.0),
                Vector4::ZERO,
                Vector4::ONE
            ),
            Vector4::new(0.0, 0.5, 1.0, 1.0)
        );
    }

    #[test]
    fn vector4_conversions_preserve_components() {
        let from_v2 = Vector4::from(Vector2::new(2.0, 3.0));
        let from_v3 = Vector4::from(Vector3::new(2.0, 3.0, 4.0));
        let from_v3w = Vector4::from((Vector3::new(2.0, 3.0, 4.0), 5.0));
        assert_eq!(from_v2, Vector4::new(2.0, 3.0, 0.0, 0.0));
        assert_eq!(from_v3, Vector4::new(2.0, 3.0, 4.0, 0.0));
        assert_eq!(from_v3w, Vector4::new(2.0, 3.0, 4.0, 5.0));
        assert!(
            Vector4::new(0.0, 0.0, 3.0, 4.0)
                .normalized()
                .approx_eq(Vector4::new(0.0, 0.0, 0.6, 0.8))
        );
    }
}
