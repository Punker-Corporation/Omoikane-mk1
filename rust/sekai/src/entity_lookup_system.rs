...
impl EntityLookupSystem {
    ...
    pub fn update_bounds(&self, manager: &mut EntityManager, entity: EntityUid) -> bool {
        ...
        if xform.anchored {
            let Some((grid_uid, tile)) = self.get_anchor_registration(manager, entity, &xform) else {
                return false;
            };
            return manager
                .map_grids
    ...
    pub fn get_entities_intersecting_world_aabb(
        ...
        if include_anchored {
            if let Some(grid) = manager.map_grids.get(&lookup_owner) {
                let tile_hits = grid
                    ...
                    .flat_map(|chunk_index| {
                        ...
                        for x in 0..chunk.chunk_size() {
                            for y in 0..chunk.chunk_size() {
                                let grid_tile = chunk.chunk_tile_to_grid_tile(Vector2i::new(x as i32, y as i32));
...
mod tests {
    ...
    fn lookup_system_removes_stale_owner_entries_and_refreshes_subtrees() {
        ...
        let first_grid_component = manager
            .map_grid_components
            .entry(first_grid)
            .or_insert_with(crate::MapGridComponent::new);
        first_grid_component.base.owner = first_grid;
        first_grid_component.grid_index = GridId::new(3);
        let first_map_grid = first_grid_component.alloc_map_grid(first_grid, MapId::new(1), 1).clone();
        ...
        let second_grid_component = manager
            .map_grid_components
            .entry(second_grid)
            .or_insert_with(crate::MapGridComponent::new);
        second_grid_component.base.owner = second_grid;
        second_grid_component.grid_index = GridId::new(4);
        let second_map_grid = second_grid_component.alloc_map_grid(second_grid, MapId::new(1), 1).clone();
        ...
        manager.transforms.get_mut(&second_grid).unwrap().local_position = keisan::Vector2::new(10.0, 0.0);
        manager.transforms.get_mut(&second_grid).unwrap().rebuild_for_manager();
