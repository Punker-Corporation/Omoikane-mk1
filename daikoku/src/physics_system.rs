...
impl PhysicsSystem {
    ...
    pub fn step_simulation(
        ...
        for uid in bodies {
            ...
            if !body.awake
                ...
                || body.linear_velocity == keisan::Vector2::ZERO
            {
                continue;
            }
            ...
            if transforms.set_local_position(
                entities,
                uid,
                current + body.linear_velocity * frame_time,
            ) {
                moved += 1;
            }
        }
    ...
    pub fn sync_map_physics(&self, entities: &mut ServerEntityManager, maps: &MapManager) -> usize {
        ...
        for map_id in maps.get_all_map_ids() {
            ...
            total += self
                .shared
                .sync_broadphase(&mut entities.inner, owner, &bodies);
            ...
            let _ = self
                .shared
                .sync_contacts(&mut entities.inner, owner, &bodies);
