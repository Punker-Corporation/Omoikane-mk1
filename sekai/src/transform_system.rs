use crate::{
    EntityCoordinates, EntityManager, EntityUid, MapId, MoveEvent, TransformComponent,
    TransformResolver, map_grid::MapGridLike,
};
...
impl SharedTransformSystem {
    ...
    pub fn get_mover_coordinates(
        ...
        if let Some(grid) = manager
            .map_grids
            .values()
            .find(|grid| grid.index == xform.grid_id)
        {
    ...
    fn process_queue(
        queue: &mut VecDeque<MoveEvent>,
        manager: &EntityManager,
        processed: &mut Vec<MoveEvent>,
    ) {
...
mod tests {
    ...
    fn transform_system_reads_world_transforms_from_manager() {
        ...
        assert_eq!(
            system.get_world_position(&manager, uid).unwrap(),
            Vector2::ZERO
        );
        assert_eq!(
            system.get_map_id(&manager, Some(EntityUid::INVALID)),
            crate::MapId::NULLSPACE
        );
    }
}
