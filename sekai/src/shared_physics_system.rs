...
impl SharedPhysicsSystem {
                    ...
                    for (fixture_b, transform_b) in fixtures_b {
                        ...
                        let index = if let Some(previous) = Self::find_previous_contact(
                            ...
                            if status != butsuri::ContactStatus::NoContact
                                && let Some(physics_map) = manager.physics_maps.get_mut(&map_owner)
                            {
                            ...
                            if let Some(status) = contact_manager.update_touching(index, true)
                                && status != butsuri::ContactStatus::NoContact
                                && let Some(contact) = contact_manager.contact_mut(index).cloned()
                                && let Some(physics_map) = manager.physics_maps.get_mut(&map_owner)
                            {
    ...
    fn compute_aabb_aabb_manifold(
        ...
        ContactManifold {
            ...
            points: vec![ContactManifoldPoint {
                local_point: transform_a.inverse_transform_point(world_point),
                normal_impulse: 0.0,
                tangent_impulse: 0.0,
            }],
        }
    ...
    fn compute_circle_circle_manifold(
        ...
        let center_a = transform_a * shape_a.position;
        let center_b = transform_b * shape_b.position;
    ...
    fn compute_aabb_circle_manifold(
        ...
        let center = transform_circle * shape_circle.position;
        ...
        let (normal, world_point) = if distance > 0.0001 {
            ...
            if left <= right && left <= bottom && left <= top {
                ...
                (
                    Vector2::new(0.0, -1.0),
                    Vector2::new(center.x, bounds.bottom),
                )
            } else {
        ...
        ContactManifold {
            normal,
            points: vec![ContactManifoldPoint {
                local_point: transform_aabb.inverse_transform_point(world_point),
                normal_impulse: 0.0,
                tangent_impulse: 0.0,
            }],
        }
    ...
    fn compute_circle_aabb_manifold(
        ...
        ContactManifold {
            ...
            points: vec![ContactManifoldPoint {
                local_point: transform_circle.inverse_transform_point(world_point),
                normal_impulse: 0.0,
                tangent_impulse: 0.0,
            }],
        }
